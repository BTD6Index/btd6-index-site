export default function TwoMPRules() {
    return <>
        <p><a href="/2mp">Back to 2MPs</a></p>
        <h1>Rules for 2MP</h1>
        <ol>
            <li>Challenge editor should be set to hard difficulty and CHIMPS mode.
Default values for Starting Cash, Staring Lives, Max Lives, Start Round, and End Round are acceptable.
<strong>Under no circumstances should your end round be set to r101, r120, r140, or r300.</strong></li>
            <li>Least Cash condition set to "No Score Set" is required for any run being submitted (you can easily reset this after winning a run by clicking auto twice)</li>
            <li>
                Please provide a screenshot of both the final setup and the victory screen (alternatively, both can be in a recording).
            </li>
            <li>Any Adora run, as well as any 2MP run on Geared/both Puddles maps/Ouch/Quad/Dark Dungeons/Ravine/Erosion/X Factor, needs full recordings.</li>
            <li>Any hard rounds need to be recorded.</li>
        </ol>
    </>;
}