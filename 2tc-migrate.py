...
with open(args.output_file, 'wb') as f:
    for entry in parsed['info']:
        f.write(f'INSERT INTO "2tc_extra_info" VALUES ({sql_escape(standardize_entity(entry["TOWER_1"]["NAME"]))}, {sql_escape(standardize_entity(entry["TOWER_2"]["NAME"]))}, {sql_escape(entry["TOWER_1"]["UPGRADE"])}, {sql_escape(entry["TOWER_2"]["UPGRADE"])}, {sql_escape(entry["VERSION"])}, {date_to_sql(entry["DATE"])});\n'
                .encode())
        for key, sub_entry in entry['MAPS'].items():
            ...
            f.write(
                f'INSERT INTO "2tc_completions" VALUES ({sql_escape(standardize_entity(entry["TOWER_1"]["NAME"]))}, {sql_escape(standardize_entity(entry["TOWER_2"]["NAME"]))}, {sql_escape(key)}, {sql_escape(sub_entry["PERSON"])}, {sql_escape(link)}, {int(sub_entry.get("OG", False))}, NULL);\n'
                .encode()
                )
