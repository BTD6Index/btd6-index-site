        ...
        context.waitUntil(
            media.list({prefix: `${imageKey}/attach`}).then(async (listRes) => {
                await fetch(webhookUrl, {
                    body: JSON.stringify({
                        "embeds": [{
                            "title": `${formData.get('completiontype').toUpperCase()} ${JSON.parse(formData.get('towerset')).length}TC on ${formData.get('map')} ${edit ? 'Edited' : 'Submitted'}${verify ? ' and Verified' : ''}`,
                            "color": 16737024,
                            "fields": [
                                {
                                    "name": "Person",
